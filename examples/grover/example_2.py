...
def oracle(qubits, num_1):
    ...
    oracle.add(sum.on_qubits(*(list(range(qubits)) + list(range(qubits + 1, sum.nqubits + 1)))))
    ...
    for i, b in enumerate(booleans[::-1]):
        if b=='0': oracle.add(gates.X(qubits + 1 + i))

    oracle.add(gates.X(qubits).controlled_by(*range(qubits + 1, sum.nqubits + 1)))

    for i, b in enumerate(booleans[::-1]):
        if b=='0': oracle.add(gates.X(qubits + 1 + i))

    oracle.add(sum.invert().on_qubits(*(list(range(qubits)) + list(range(qubits + 1, sum.nqubits + 1)))))
